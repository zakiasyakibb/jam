...
<body>
    ...
    <!-- Google Fonts: Inter & JetBrains Mono untuk angka digital -->
...
<body class="bg-[#0b0f17] text-gray-100 min-h-screen flex flex-col justify-between overflow-x-hidden">
    ...
    <header class="w-full max-w-5xl mx-auto px-4 py-5 flex justify-between items-center z-10">
        <div class="flex items-center gap-3">
            <div class="p-2.5 bg-brand-500/10 rounded-xl border border-brand-500/30 text-brand-500 animate-pulse">
                <!-- SVG Icon Clock -->
            ...
            <div>
                <h1 class="text-lg font-bold tracking-tight text-white">Syrven<span class="text-brand-500">Sync</span></h1>
        ...
        <div id="sync-status-badge" class="flex items-center gap-2 px-3 py-1.5 rounded-full bg-amber-500/10 border border-amber-500/30 text-amber-400 text-xs font-medium cursor-pointer transition-all duration-300 hover:bg-amber-500/20" onclick="sinkronisasiWaktu()">
            <span class="relative flex h-2 w-2">
                <span id="sync-dot-ping" class="animate-ping absolute inline-flex h-full w-full rounded-full bg-amber-400 opacity-75"></span>
                <span id="sync-dot" class="relative inline-flex rounded-full h-2 w-2 bg-amber-500"></span>
            </span>
    ...
    <main class="flex-1 w-full max-w-5xl mx-auto px-4 flex flex-col justify-center py-6">
        ...
        <div class="flex justify-center mb-8 overflow-x-auto pb-2">
            <div class="bg-gray-900/80 p-1 rounded-2xl border border-gray-800 flex gap-1 shrink-0">
                <button onclick="switchTab('jam')" id="tab-jam" class="px-5 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 bg-brand-500 text-white shadow-lg shadow-brand-500/20 whitespace-nowrap">
                    Jam Utama
                </button>
                <button onclick="switchTab('dunia')" id="tab-dunia" class="px-5 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 text-gray-400 hover:text-white whitespace-nowrap">
                    Jam Dunia
                </button>
                <button onclick="switchTab('stopwatch')" id="tab-stopwatch" class="px-5 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 text-gray-400 hover:text-white whitespace-nowrap">
                    Stopwatch
                </button>
                <button onclick="switchTab('timer')" id="tab-timer" class="px-5 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 text-gray-400 hover:text-white whitespace-nowrap">
                    Timer
                </button>
                <button onclick="switchTab('ai')" id="tab-ai" class="px-5 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 text-gray-400 hover:text-white whitespace-nowrap flex items-center gap-1.5">
                    ✨ Asisten AI
                </button>
            </div>
        </div>
        ...
        <div id="content-jam" class="tab-content space-y-8">
            <div class="grid grid-cols-1 md:grid-cols-12 gap-8 items-center">
                ...
                <div class="md:col-span-5 flex justify-center">
                    <div class="relative w-64 h-64 md:w-80 md:h-80 rounded-full glass flex items-center justify-center p-4 border border-gray-800 shadow-2xl">
                        ...
                        <div class="absolute inset-4 text-xs font-semibold text-gray-500">
                            ...
                            <span class="absolute right-1 top-1/2 -translate-y-1/2">3</span>
                ...
                <div class="md:col-span-7 space-y-6 text-center md:text-left">
                    <div class="space-y-2">
                        <span class="text-xs font-semibold uppercase tracking-widest text-brand-500 px-3 py-1 bg-brand-500/10 rounded-full border border-brand-500/20">
                            Waktu Lokal Akurat
                        </span>
                        <!-- Digital Time Display -->
                        <div class="text-5xl sm:text-6xl md:text-7xl font-bold font-mono-custom tracking-tight text-white pt-2 flex justify-center md:justify-start items-baseline">
                            <span id="digital-time">00:00:00</span>
                            <span id="digital-ms" class="text-xl sm:text-2xl md:text-3xl text-brand-500/80 ml-2">.000</span>
                        </div>
                        <!-- Date Display -->
                        <div id="date-display" class="text-gray-400 text-lg sm:text-xl font-medium">
                            Memuat tanggal...
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 gap-4 max-w-md mx-auto md:mx-0">
                        <div class="glass p-3 rounded-xl border border-gray-800 text-left">
                            <p class="text-[10px] uppercase tracking-wider text-gray-400">Selisih (Offset)</p>
                            <p id="stat-offset" class="text-sm font-semibold text-white font-mono-custom">Calculating...</p>
                        </div>
                        <div class="glass p-3 rounded-xl border border-gray-800 text-left">
                            <p class="text-[10px] uppercase tracking-wider text-gray-400">Latensi Jaringan</p>
                            <p id="stat-latency" class="text-sm font-semibold text-white font-mono-custom">Calculating...</p>
                        </div>
                        <div class="glass p-3 rounded-xl border border-gray-800 text-left col-span-2 sm:col-span-1">
                            <p class="text-[10px] uppercase tracking-wider text-gray-400">Sumber Sinkronisasi</p>
                    ...
                    <div class="flex flex-wrap justify-center md:justify-start gap-3">
                        ...
                        <button onclick="toggleMilliseconds()" id="btn-toggle-ms" class="px-5 py-2.5 bg-gray-800 hover:bg-gray-700 active:scale-95 text-gray-300 font-medium rounded-xl text-sm transition-all duration-150">
                            Sembunyikan Milidetik
                        </button>
                    </div>
                </div>
            </div>
        ...
        <div id="content-stopwatch" class="tab-content hidden space-y-6 max-w-xl mx-auto">
            <div class="text-center space-y-4">
                ...
                <div class="flex justify-center gap-4">
                    ...
                    <button onclick="startStopwatch()" id="btn-stopwatch-start" class="px-8 py-3 bg-brand-500 hover:bg-brand-600 text-white font-medium rounded-2xl text-sm transition-all duration-150 flex items-center gap-2 shadow-lg shadow-brand-500/20">
                        Mulai
                    </button>
                    <button onclick="resetStopwatch()" id="btn-stopwatch-reset" class="px-6 py-3 bg-red-500/10 hover:bg-red-500/20 text-red-400 font-medium rounded-2xl text-sm transition-all duration-150 border border-red-500/20">
                        Reset
                    </button>
                </div>
                ...
                <div class="mt-6 text-left">
                    ...
                    <div id="stopwatch-laps" class="max-h-48 overflow-y-auto space-y-2 pr-2">
                        <p class="text-sm text-gray-500 text-center py-4">Belum ada putaran yang direkam.</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div id="content-timer" class="tab-content hidden space-y-6 max-w-xl mx-auto">
            <div class="text-center space-y-6">
                ...
                <div id="timer-inputs" class="glass p-6 rounded-3xl border border-gray-800 space-y-4">
                    ...
                    <div class="flex justify-center items-center gap-4">
                        <div class="flex flex-col items-center">
                            <input type="number" id="timer-hour" min="0" max="23" value="0" class="w-16 h-16 text-2xl font-bold text-center bg-gray-900 border border-gray-800 rounded-2xl focus:border-brand-500 focus:outline-none text-white font-mono-custom">
                            <span class="text-[10px] text-gray-400 mt-1 uppercase">Jam</span>
                        </div>
                        <span class="text-xl font-bold text-gray-600">:</span>
                        <div class="flex flex-col items-center">
                            <input type="number" id="timer-min" min="0" max="59" value="5" class="w-16 h-16 text-2xl font-bold text-center bg-gray-900 border border-gray-800 rounded-2xl focus:border-brand-500 focus:outline-none text-white font-mono-custom">
                            <span class="text-[10px] text-gray-400 mt-1 uppercase">Menit</span>
                        </div>
                        <span class="text-xl font-bold text-gray-600">:</span>
                        <div class="flex flex-col items-center">
                            <input type="number" id="timer-sec" min="0" max="59" value="0" class="w-16 h-16 text-2xl font-bold text-center bg-gray-900 border border-gray-800 rounded-2xl focus:border-brand-500 focus:outline-none text-white font-mono-custom">
                            <span class="text-[10px] text-gray-400 mt-1 uppercase">Detik</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex justify-center gap-4">
                    ...
                    <button onclick="resetTimer()" id="btn-timer-reset" class="px-6 py-3 bg-red-500/10 hover:bg-red-500/20 text-red-400 font-medium rounded-2xl text-sm transition-all duration-150 border border-red-500/20 hidden">
                        Reset
                    </button>
                </div>
            </div>
        ...
        <div id="content-ai" class="tab-content hidden space-y-8">
            <div class="text-center md:text-left space-y-2 max-w-xl">
                <h2 class="text-2xl font-bold flex items-center justify-center md:justify-start gap-2 text-white">
                    ✨ Asisten AI SyrvenSync
                </h2>
                    ...
                    <div>
                        ...
                        <div class="mt-4 space-y-3">
                            <div>
                                ...
                                <input type="time" id="ai-start-time" value="08:00" class="w-full bg-gray-900 border border-gray-800 rounded-xl p-2.5 text-white text-sm focus:outline-none focus:border-brand-500 font-mono-custom">
                            </div>
                            <div>
                                <label class="text-xs text-gray-400 block mb-1 font-medium">Daftar Kegiatan/Tugas:</label>
                                <textarea id="ai-tasks" rows="4" placeholder="Contoh:&#10;- Olahraga pagi&#10;- Meeting dengan tim marketing&#10;- Belajar coding JavaScript&#10;- Belanja bulanan" class="w-full bg-gray-900 border border-gray-800 rounded-xl p-3 text-white text-sm focus:outline-none focus:border-brand-500 placeholder-gray-600 resize-none"></textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="mt-4 space-y-3">
                            <div>
                                ...
                                <input type="text" id="ai-timezone-city" placeholder="Contoh: Jepang, Spanyol, Meksiko, dll." class="w-full bg-gray-900 border border-gray-800 rounded-xl p-2.5 text-white text-sm focus:outline-none focus:border-brand-500">
                            </div>
                            <div class="bg-gray-900/40 p-3 rounded-xl border border-gray-800/60">
                                <p class="text-[11px] text-gray-400 leading-relaxed">
                                    <span class="text-brand-500 font-semibold">Tahu kah Anda?</span> Sebagian kebudayaan memandang waktu sebagai garis lurus (monokronik), sedangkan yang lain memandangnya sebagai siklus yang santai (polikronik). Mari cari tahu bagaimana tempat pilihan Anda memaknai waktu!
                                </p>
                            </div>
                        </div>
                    </div>
            ...
            <div id="ai-output-panel" class="glass p-6 rounded-3xl border border-gray-800 space-y-4 hidden transition-all duration-300">
                <div class="flex justify-between items-center border-b border-gray-800 pb-3">
                    <h3 class="font-bold text-white flex items-center gap-2">
                        <span class="text-brand-500">🤖</span> Hasil Analisis AI
                    </h3>
    ...
    <div id="custom-toast" class="fixed bottom-6 right-6 transform translate-y-20 opacity-0 pointer-events-none transition-all duration-300 z-50">
        <div class="glass px-5 py-3.5 rounded-2xl border border-brand-500/30 flex items-center gap-3 shadow-xl">
            <div id="toast-icon-container" class="p-1.5 bg-brand-500/20 text-brand-400 rounded-lg">
                <!-- Check SVG Icon -->
            ...
            <div>
                <p id="toast-title" class="text-sm font-semibold text-white">Judul Notifikasi</p>
                <p id="toast-desc" class="text-xs text-gray-400">Deskripsi pesan di sini.</p>
            </div>
        </div>
    </div>
    <!-- Script Utama -->
    <script>
        ...
        async function sinkronisasiWaktu() {
            ...
            if (!success) {
                ...
                statOffset.innerText = '0 ms (Lokal)';
                statSource.innerText = 'Internal Lokal';
                updateSyncUI('gagal');
        ...
        function updateSyncUI(state) {
            ...
            } else if (state === 'berhasil') {
                badge.className = "flex items-center gap-2 px-3 py-1.5 rounded-full bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-xs font-medium cursor-pointer transition-all duration-300 hover:bg-emerald-500/20";
                dot.className = "relative inline-flex rounded-full h-2 w-2 bg-emerald-500";
                dotPing.className = "animate-ping absolute inline-flex h-full w-full rounded-full bg-amber-400 opacity-75";
                text.innerText = "Terbaca Akurat";
            } else if (state === 'gagal') {
                badge.className = "flex items-center gap-2 px-3 py-1.5 rounded-full bg-red-500/10 border border-red-500/30 text-red-400 text-xs font-medium cursor-pointer transition-all duration-300 hover:bg-red-500/20";
        ...
        function toggleMilliseconds() {
            ...
            if (showMs) {
                btn.innerText = "Sembunyikan Milidetik";
                btn.className = "px-5 py-2.5 bg-gray-800 hover:bg-gray-700 active:scale-95 text-gray-300 font-medium rounded-xl text-sm transition-all duration-150";
            } else {
                btn.innerText = "Tampilkan Milidetik";
        ...
        const worldCities = [
            ...
            { name: "Tokyo", timezone: "Asia/Tokyo", label: "JST (GMT+9)" },
            { name: "London", timezone: "Europe/London", label: "BST/GMT (GMT+1)" },
            { name: "New York", timezone: "America/New_York", label: "EDT (GMT-4)" },
                ...
                grid.innerHTML += `
                    <div class="glass p-5 rounded-2xl border border-gray-800 flex justify-between items-center hover:border-brand-500/30 transition-all duration-200">
                        <div>
                            ...
                            <p class="text-xs text-gray-400">${city.label}</p>
                        </div>
                        <div class="text-right">
                            <p id="world-time-${index}" class="text-xl font-bold font-mono-custom text-brand-500">00:00:00</p>
                            <p id="world-date-${index}" class="text-[10px] text-gray-400">Memuat...</p>
                        </div>
                    </div>
                `;
            });
            ...
            stopwatchLaps.forEach((lapTime, index) => {
                ...
                container.innerHTML += `
                    <div class="flex justify-between items-center py-2 px-3 bg-gray-900/40 rounded-xl border border-gray-800">
                        ...
                        <span class="text-sm font-mono-custom text-white font-bold">${timeString}</span>
                    </div>
                `;
            });
        }
        ...
        function startTimer() {
            ...
            if (!timerRunning) {
                ...
                btn.innerText = "Jeda Timer";
                btn.className = "px-8 py-3 bg-amber-500 hover:bg-amber-600 text-white font-medium rounded-2xl text-sm transition-all duration-150 flex items-center gap-2 shadow-lg shadow-amber-500/20";
            } else if (btn.innerText === "Jeda Timer") {
                // Jeda timer
        ...
        function showToast(title, desc, status = 'sukses') {
            ...
            if (status === 'gagal') {
                ...
                iconContainer.className = "p-1.5 bg-brand-500/20 text-brand-400 rounded-lg";
                iconContainer.innerHTML = `
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" />
                    </svg>
        ...
        function createClockTicks() {
            ...
            for (let i = 0; i < 60; i++) {
                ...
                if (i % 5 === 0) {
                    ...
                    marker.className = 'w-[1px] h-1.5 bg-gray-600';
                }
            ...
            for (let attempt = 1; attempt <= maxRetries; attempt++) {
                try {
                    ...
                    if (attempt === maxRetries) {
                        throw new Error("Gagal terhubung dengan server kecerdasan buatan. Silakan periksa koneksi internet Anda.");
                    }
                    // Menunggu dengan delay eksponensial (1s, 2s, 4s, 8s, 16s)
        ...
        async function generateSchedule() {
            ...
            try {
                ...
                showToast("Jadwal Berhasil Dibuat", "AI telah memetakan waktu harian Anda secara presisi.");
            } catch (error) {
        ...
        async function exploreCulturalTime() {
            ...
            try {
                const response = await callGemini(prompt, systemPrompt);
                displayAIResult(response);
                showToast("Wawasan Ditemukan", `Eksplorasi budaya waktu di ${location} berhasil dimuat.`);
            } catch (error) {
                displayAIError(error.message);
            }
        }
        ...
        function copyAIResult() {
            ...
            if (!resultText || resultText.includes('⚠️ Terjadi Kesalahan') || resultText.includes('Menghubungkan')) {
            ...
            try {
                ...
                showToast("Berhasil Disalin", "Analisis waktu AI telah disimpan ke clipboard Anda.");
            } catch (err) {
                showToast("Gagal Menyalin", "Browser tidak mengizinkan operasi ini.", "gagal");
            }
